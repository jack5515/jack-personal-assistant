...
| `wechat-feishu-summary` | 部分可用 | `scripts/run-wechat-feishu-summary.py` | 本地微信消息 API、飞书目标配置 | `workfiles/wechat-feishu-summary-latest.md` | `memory/wechat-feishu-summary-state.json`, `logs/wechat-feishu-summary.log` | `scripts/workbench-verify.sh wechat-summary` | `localhost:6174` 连接失败、消息去重基线错位、兴趣阈值误筛导致噪音或漏报 |
...
| `effect-review` | 部分可用 | `docs/effect-review-template.md` | recent outputs, user outcomes | short effect review | `docs/effect-review-template.md` | `scripts/workbench-verify.sh docs` | 只复盘流程，不复盘是否真的帮助 Jack 更强 |
| `output-calibration-log` | 部分可用 | `docs/output-calibration-log.md` | v1 outputs, Jack-edited v2 outputs | reusable calibration patterns | `docs/output-calibration-log.md` | `scripts/workbench-verify.sh docs` | 把所有问题误归因给 prompt、没有把差异更新到正确层 |
...
scripts/workbench-verify.sh wechat-summary
